...
class Zender {
  ...
  parseEvent(node) {
    if(node.getAttribute('zen-event')) {
      const eventName = node.getAttribute('zen-event')
  ...
  parseClass(node) {
    if(node.getAttribute('zen-class')) {
      const className = node.getAttribute('zen-class')
  ...
  parseModel(node) {
    if(node.getAttribute('zen-model')) {
      const modelName = node.getAttribute('zen-model')
  ...
  parseList(node) {
    ...
    const _list = node.getAttribute('zen-list')
